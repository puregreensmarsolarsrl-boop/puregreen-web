...
const newsCards=()=>{
  const items=Array.isArray(content.noticias?.items)?content.noticias.items:[];
  return items.map(item=>`<article class="news-card">${image(item)?`<div class="news-image">${image(item)}</div>`:''}<div class="news-copy"><span class="news-date">${escape(item.fecha||'Actualidad')}</span><h3>${escape(item.titulo||'Noticia solar')}</h3><p>${escape(item.resumen||'')}</p>${item.url?`<a class="btn btn-outline" href="${escape(item.url)}" target="_blank" rel="noopener noreferrer">Leer noticia completa ↗</a>`:''}</div></article>`).join('\n');
};
...
let output=template.replace(/\{\{([\w.]+)\}\}/g,(_,key)=>{
  ...
  if(key==='noticias')return newsCards();
...
.social-strip{background:#071b14;color:#eaf8f0;padding:12px 0;font-size:13px}.social-strip-inner{display:flex;align-items:center;justify-content:space-between;gap:18px}.social-links{display:flex;flex-wrap:wrap;gap:14px}.social-links a{color:#c8f26f;font-weight:800}.social-link{display:inline-flex;align-items:center;gap:6px}.social-icon{width:18px;height:18px;display:block;flex:none}.location-grid{display:grid;grid-template-columns:.85fr 1.15fr;gap:28px;align-items:stretch}.map-frame{min-height:340px;border-radius:22px;overflow:hidden;box-shadow:var(--shadow);background:#dfeee6}.map-frame iframe{width:100%;height:100%;min-height:340px;border:0}
.news-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:20px}.news-card{background:#fff;border:1px solid var(--line);border-radius:20px;overflow:hidden;box-shadow:0 12px 30px rgba(7,27,20,.08);display:flex;flex-direction:column}.news-image{height:180px;background:#dfeee6}.news-image img{width:100%;height:100%;object-fit:cover}.news-copy{padding:20px;display:flex;flex-direction:column;align-items:flex-start;flex:1}.news-date{font-size:12px;font-weight:800;color:var(--green);text-transform:uppercase;letter-spacing:.07em}.news-copy h3{margin:8px 0;font-size:21px;line-height:1.15;color:var(--dark)}.news-copy p{margin:0 0 18px;color:var(--muted);flex:1}.news-copy .btn{padding:10px 14px;font-size:13px}
</style></head>`);
